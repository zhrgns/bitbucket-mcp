...
const loadCredentialsFromZshrc = (): Credentials | null => {
...
const requireRepoField = (value: string | undefined, field: string): string => {
  const trimmed = value?.trim();
  if (!trimmed) {
    throw new Error(
      `Missing repository.${field} in ${getConfigPath()}. Copy examples/config.json and run npm run init-config`
    );
  }
  return trimmed;
};
...
export const loadConfig = (): McpConfig => {
  ...
  if (!fs.existsSync(configPath)) {
    throw new Error(
      `Config not found at ${configPath}. Run: npm run init-config (from repo) or copy examples/config.json`
    );
  }
  ...
  cachedConfig = {
    ...
    reviewers: {
      useEffectiveDefaultReviewers:
        raw.reviewers?.useEffectiveDefaultReviewers ??
        DEFAULT_REVIEWERS.useEffectiveDefaultReviewers,
      includeAuthorAsReviewer:
        raw.reviewers?.includeAuthorAsReviewer ??
...
export const getRepoApiPrefix = (config: McpConfig): string =>
  `https://api.bitbucket.org/2.0/repositories/${config.repository.workspace}/${config.repository.slug}`;
